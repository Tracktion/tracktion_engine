...
struct EditNodeBuilder
{
    ...
    */
    static std::function<std::unique_ptr<graph::Node> (std::unique_ptr<graph::Node>)> insertOptionalLastStageNode;
};

//==============================================================================
/** Creates a Node to play back an Edit with live inputs and outputs. */
std::unique_ptr<graph::Node> createNodeForEdit (EditPlaybackContext&, std::atomic<double>& audibleTimeToUpdate, const CreateNodeParams&);

/** Creates a Node to render an Edit. */
std::unique_ptr<graph::Node> createNodeForEdit (Edit&, const CreateNodeParams&);
