...
namespace BeatConnect 
{
	...
	const struct midiCalibriation
	{
		inline static const double hz = 440.0;
		inline static const int midiNote = 69;
		inline static const String noteName = "A4";
	};
	...
	const struct MidiNote 
	{
		...
		inline static const std::vector<midiNoteValue> midiNoteValues =
		{
			{0, 8.1757989156, "C1"},
			{1, 8.6619572180, "Db1"},
			{2, 9.1770239974, "D1"},
		...
		{
			return p_noteHz * pow(2.0, ((p_pitchWheelPosition - (double)BeatConnect::midiPitchWheelBase) * p_pitchWheelSemitoneRange) /
				(BeatConnect::semitonesPerOctave * (double)BeatConnect::midiPitchWheelBase));
		}
		...
		static int getMidiNote(String p_name) 
		{
			for (auto element : midiNoteValues)
		...
		static double getNoteFrequency(String p_noteName)
		{
			for (auto element : midiNoteValues) 
		...
		static String getNoteName(double p_hz) 
		{
			for (auto element : midiNoteValues) 
			{
				if (element.freqHz == p_hz)
		...
		static String getNoteName(unsigned int p_midiNote)
		{
			for (auto element : midiNoteValues) 
				if (element.midiNote == p_midiNote)
					return element.noteName;
